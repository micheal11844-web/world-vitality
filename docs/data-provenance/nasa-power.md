...
**CC BY 4.0** (Creative Commons Attribution 4.0), confirmed directly from
NASA POWER's own published project metadata
(https://power.larc.nasa.gov/llms.txt: "License: CC BY 4.0"). This
supersedes an earlier version of this doc, which assumed public domain by
default (the common case for U.S. government works) without checking
POWER's specific stated policy — worth noting as a reminder that "NASA
data" isn't a single blanket license; POWER states its own explicitly, and
it's not the default.

CC BY 4.0 requires attribution (crediting NASA POWER, linking the license)
but otherwise permits free use, including commercial and derivative use.
See https://power.larc.nasa.gov/docs/referencing/ for POWER's specific
citation guidance.

Recorded in every record's `provenance.license` field as `"CC-BY-4.0"`,
with `provenance.attributionUrl` pointing at the "How to Cite POWER" page.
