...
/** Same band boundaries as SoilMoistureStatusProvider/Agriculture's
 *  MapView, kept in sync manually — a display concern, not logic
 *  worth importing a whole service for. */
 ...
 * WORKSPACE"). Same "base layers + one data overlay" pattern as every
 * other workspace's MapView — single demo-jurisdiction point, colored
 * by soil moisture band. No real jurisdiction boundary polygon (this
 * app has no GIS boundary data) — see `page.tsx`'s honest-scope notes.
 */
export function MapView({ latitude, longitude, moistureValue }: MapViewProps) {
  ...
  const description =
    overlayOn && moistureValue !== undefined
      ? `Map centered on the demo jurisdiction point. Soil moisture overlay shows a value of ${moistureValue.toFixed(2)} at the marker.`
      : "Map centered on the demo jurisdiction point. Data overlay is currently hidden.";
