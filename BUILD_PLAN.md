...
## STAGE 10 — WEATHER & CLIMATE (second workspace) — IN PROGRESS
...
- [ ] **10.5** Map view for Weather & Climate — **not built**. The PRD describes one (standard weather/climate overlay layers); this pass deliberately scoped to the status widget only, following Stage 4's own "narrow scope first" precedent. Honestly left open, not silently dropped.
- [ ] **10.6** Forecast / short-medium-long-range confidence gradient — **not built, and not fakeable yet**. `WeatherStatusProvider` is current-conditions-only; NASA POWER's Daily API has no forward-looking forecast data. A real forecast needs an actual forecast data source (a new ingestion connector), which is real, separate, un-started work — see `WeatherStatusProvider`'s own doc comment.

**Verified so far:** 88/88 tests passing, clean lint, clean format, clean production `next build` (`/workspaces/weather` builds correctly as a dynamic route, matching Agriculture's pattern).
...
- Second **data-provider** connector (a genuinely different provider, e.g. ESA/NOAA — not yet done; Stage 10 validated the ingestion/interpretation boundary with a second _parameter_ through the _same_ provider, which is real evidence but not the same test as a structurally different provider's data shape)
- Mobile app (`apps/mobile`)
- Admin console (`apps/admin`)
- Billing/subscriptions (`services/billing-service`)
- Any workspace beyond Agriculture and Weather & Climate (9 of the PRD's 11 workspaces remain un-started; Disaster Monitoring in particular should not be started without deliberately revisiting its "zero tolerance" life-safety requirements first)
